...
# initial data (reference data that we might like to make flexible by putting in the DB but users can't change)
do_initial=false
...
show_help() {
    ...
    #echo " --no-initial    Don't load initial data fixtures"
...
! $do_fixtures || ! $do_initial || ./manage.py loaddata initial
! $do_fixtures || ! $do_groups  || ./manage.py loaddata groups
! $do_fixtures || ! $do_dev     || ./manage.py loaddata users
