...
function lint_python_version() {
	...
	python_version_pip=$( sed -E 's/^python-version-(([0-9]+\.)*[0-9]+).*$/\1/p;d' < requirements.txt )
	...
	python_version_pip_minor="$( sed -E 's/^python-version-([0-9]+\.[0-9]+).*$/\1/p;d' < requirements.txt )."
...
function lint_python_packages() {
	...
	if grep '^pytz[^$]' requirements/*.txt >/dev/null ; then
